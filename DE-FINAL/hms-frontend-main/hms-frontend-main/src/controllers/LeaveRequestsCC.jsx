...
const LeaveRequestsCC = () => {
  ...
  const fetchLeaveRequests = async () => {
    try {
      const response = await axios.get(`/coordinator/leave-requests`);
  ...
  const handleStatusChange = async (requestId, status) => {
    try {
      await axios.post(`/coordinator/leave-requests/${requestId}`, {
        status,
      });
      const leaveRequestsData = await fetchLeaveRequests();
      if (leaveRequestsData && leaveRequestsData.length > 0) {
...
export default LeaveRequestsCC;
